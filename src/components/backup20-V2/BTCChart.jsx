...
const BTCChart = () => {
  ...
  const height = 300;
  const centerY = height / 2; // <-- Move this up here!
  ...
  const scaleY = useCallback(val => {
    if (open1Min === null) return centerY;
    const diff = val - open1Min;
    const pixelsPerUnit = (height - PADDING * 2) / yRange;
    // Calculate unclamped Y
    const unclampedY = centerY - diff * pixelsPerUnit;
    // Clamp to chart area
    return Math.max(PADDING, Math.min(height - PADDING, unclampedY));
  }, [open1Min, yRange, height]);
  ...
  const yLabels = useMemo(() => {
    ...
    for (let i = 0; i <= steps; i++) {
      // y position from top (PADDING) to bottom (height - PADDING)
      const y = PADDING + ((height - PADDING * 2) * i) / steps;
      // value at this y position
      const value = open1Min !== null
        ? open1Min - ((y - centerY) / ((height - PADDING * 2) / yRange))
        : 0;
      labels.push({ value, y });
    }
    return labels;
  }, [open1Min, yRange, height, centerY]);
    ...
    if (open1Min !== null) {
      ...
      const interval = setInterval(() => {
        setCountdown(prev => {
          ...
          // Optionally: trigger next open here, or reset to 60 if looping
          return 0;
        });
      }, 1000);
  ...
  useEffect(() => {
    if (countdown === 60) {
      ...
      const timeout = setTimeout(() => setCountdownAnimate(false), 700);
          ...
          <>
            ...
            <g className="btc-open-label-group">
              ...
              >
                ...
                <tspan
                  className={`btc-countdown${countdownAnimate ? ' animate' : ''}${countdown <= 10 ? ' last10' : ''}`}
                  dx="8"
                >
                  {countdown}s
                </tspan>
